...
var knex = require('knex')({
  client: 'pg',
  connection: 'postgres://' + dbUsername + ':' +
    dbPassword + '@localhost:5432/plantdata',
...
function createTable(tableName) {
  return knex.schema.createTable(tableName, function(tableName) {
    var column;
    var columnKeys = _.keys(Schema[tableName]);
    _.each(columnKeys, function(key) {
      if (Schema[tableName][key].type === 'text' &&
      Schema[tableName][key].hasOwnProperty('fieldtype')) {
        column = table[Schema[tableName][key].type](
          key, Schema[tableName][key].fieldtype
        );
      ...
      Schema[tableName][key].hasOwnProperty('maxlength')) {
        ...
        column = tableName[Schema[tableName][key].type](key);
      }
